 ...
 * Joomla! Framework
 *
 * Modified by Achim Fischer
 *
 * @author		Louis Landry <louis.landry@joomla.org>
 * @author		Achim Fischer <achim.fischer@codingfish.com>
 * @package 	Joomla.Framework
 * @subpackage	HTML
 * @since		1.5
 */
class JPagination extends JObject
{
	...
	/**
	 * Searchtext for search in category
	 *
	 * @access public
	 * @var String
	 */
	...
	/**
	 * headline for advanced search
	 *
	 * @access public
	 * @var String
	 */
	var $search_headline = null;

	/**
	 * text for advanced search
	 *
	 * @access public
	 * @var String
	 */
	...
	/**
	 * ad_condition for advanced search
	 *
	 * @access public
	 * @var String
	 */
	...
	function _item_active(&$item)
	{
		...
		// add limit and total to link
		if ( strpos( $item->link, 'limit=') == false ) {
			$item->link .= "&limit=".$this->limit;
		}

		if ( strpos( $item->link, 'total=') == false ) {
			$item->link .= "&total=".$this->total;
		}


		// search in category
		if ( strpos( $item->link, 'searchtext=') == false ) {
			if ( $this->searchtext != null) {
				$item->link .= "&searchtext=".$this->searchtext;
			}
		}

		// type
		if ( strpos( $item->link, 'ad_type=') == false ) {
			if ( $this->ad_type != null) {
				$item->link .= "&ad_type=".$this->ad_type;
			}
		}
		// category
		if ( strpos( $item->link, 'category=') == false ) {
			if ( $this->search_category != null) {
				$item->link .= "&category=".$this->search_category;
			}
		}
		// headline
		if ( strpos( $item->link, 'ad_headline=') == false ) {
			if ( $this->search_headline != null) {
				$item->link .= "&ad_headline=".$this->search_headline;
			}
		}
		// text
		if ( strpos( $item->link, 'ad_text=') == false ) {
			if ( $this->search_text != null) {
				$item->link .= "&ad_text=".$this->search_text;
			}
		}
		// zip
		if ( strpos( $item->link, 'zip=') == false ) {
			if ( $this->search_zip != null) {
				$item->link .= "&zip=".$this->search_zip;
			}
		}
		// city
		if ( strpos( $item->link, 'city=') == false ) {
			if ( $this->search_city != null) {
				$item->link .= "&city=".$this->search_city;
			}
		}
		// state
		if ( strpos( $item->link, 'state=') == false ) {
			if ( $this->search_state != null) {
				$item->link .= "&state=".$this->search_state;
			}
		}
		// country
		if ( strpos( $item->link, 'country=') == false ) {
			if ( $this->search_country != null) {
				$item->link .= "&country=".$this->search_country;
			}
		}
		// ad_condition
		if ( strpos( $item->link, 'ad_condition=') == false ) {
			if ( $this->search_condition != null) {
				$item->link .= "&ad_condition=".$this->search_condition;
			}
		}
